...
export default async function MapPage({ searchParams }: { searchParams: Promise<{ studentId?: string }> }) {
  ...
  return <main className="mx-auto max-w-[1400px] px-8 py-14"><div className="mb-10 flex items-end justify-between"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-teal-300">Personal learning graph</p><h1 className="text-4xl font-black tracking-[-0.04em] text-white">Your misconception map</h1><p className="mt-3 max-w-2xl text-slate-400">See what you know, spot the patterns holding you back, and watch them fade as your understanding grows.</p></div>{students.length ? <form className="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-[#111A2E] p-1.5"><select className="h-11 rounded-xl bg-transparent px-4 pr-10 text-sm font-semibold text-slate-200" defaultValue={studentId} name="studentId">{students.map((student) => <option key={student.id} value={student.id}>{student.name}</option>)}</select><button className="h-11 rounded-xl bg-teal-300 px-4 text-sm font-extrabold text-[#08131d]">View map</button></form> : null}</div><MisconceptionGraph data={getStudentMap(db, studentId)} /></main>;
}
