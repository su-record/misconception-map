...
export async function createTargetedLesson(db: Database.Database, studentId: number) {
  ...
  const lesson = await generateLesson(lessonPrompt(target.name, target.description));
